...
class MyAIImageTemplateSeeder5 extends Seeder
{
    /**
     * Run the database seeds.
     * Mẫu Upload Lên S3
     * php artisan db:seed --class=MyAIImageTemplateSeeder5
     */
        ...
        try {
            foreach ($categories as $category) {
                ...
                for ($index = 1; $index <= $category['total']; $index++) {
                    $fileName = "{$category['slug']}_{$index}.jpeg";
                    $localPath = "{$category['path']}/{$fileName}";
                    $s3Path = "image_generate/{$fileName}";

                    if (file_exists(base_path($localPath))) {
                        // Upload file lên S3
                        Storage::disk('s3')->put($s3Path, file_get_contents(base_path($localPath)));
                        ...
                        echo "Uploaded and saved: {$s3Path}\n";
                    } else {
