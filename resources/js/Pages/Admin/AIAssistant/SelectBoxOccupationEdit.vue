...
<script setup>
    ...
    watch(searchQuery, (newVal, oldVal) => {
        console.log("isManualChange", isManualChange.value);
        console.log("selectedOccupation", selectedOccupation.value);
        console.log("newVal", newVal);

        // Nếu là lần khởi tạo mặc định, bỏ qua logic reset
        if (!isManualChange.value) {
            isManualChange.value = true;
            return;
        }

        // Logic xử lý khi giá trị thực sự thay đổi
        if (!selectedOccupation.value || newVal !== selectedOccupation.value.name) {
            selectedOccupation.value = null;
            emit('update:modelValue', null);
            console.log("Reset occupation", "Value set to null");
        }
