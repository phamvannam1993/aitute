            ...
            <div
                ...
                <div class="w-full lg:w-3/4 flex flex-col space-y-2">
                    <div class="flex items-center space-x-2 text-white">
                        ...
                        <span class="font-medium text-white">/ Edit Key</span>
                    </div>
                </div>
            </div>
...
<script setup>
    ...
    const { keyCreate, configAffs } = defineProps({
        keyCreate: Object,
        configAffs: Array,
    });
    const form = ref({
        configAff: keyCreate.config_aff_id || 1,
    });

    console.log("keyCreate", keyCreate)
    const handleSubmit = () => {
        // Gửi dữ liệu cập nhật đến server
        // Dùng router của InertiaJS (nếu cần)
        router.post(route('admin.keys.update', { key_id: keyCreate.key_id }), form.value, {
            onSuccess: () => {
                toast.success("Key đã được cập nhật thành công!");
            },
            onError: (errors) => {
                console.error(errors); // Xử lý lỗi
            },
        });
    };
</script>
