<template>
    <h1 class="text-3xl font-bold mb-8 text-gray-800">Cài đặt Chat bot</h1>
    <div class="flex flex-col mx-auto lg:p-6 bg-white rounded-3xl shadow-lg">
        <div class="grid grid-cols-1 gap-8">
            <div class="p-6 rounded-lg">
                <div class="grid grid-cols-12 items-center">
                    <h2 class="col-span-2 text-xl font-semibold text-gray-700">Giọng điệu trả lời</h2>
                    <form class="col-span-10 flex flex-1 grid grid-cols-10" action="" @submit.prevent="submitForm">
                        <select v-model="toneConfig" class="col-span-8 bg-white block text-[14px] appearance-none w-full text-black px-4 pr-8 rounded-[10px] focus:outline-none focus:bg-white focus:border-gray-500">
                            <option v-for="(tone, index) in toneConfigs" :value="index">{{ tone }}</option>
                        </select>
                        <button class="col-span-2 bg-ai3goc-sec text-white rounded-xl w-full py-3 hover:opacity-90 h-fit ml-5">
                            Lưu
                        </button>
                    </form>
                </div>
        ...
        <hr class="border-[1px] mt-2"/>
        <div class="lg:p-6 rounded-lg mt-5">
            <div class="flex items-center px-6 lg:px-0">
                <h2 class="text-xl font-semibold text-gray-700">Tài liệu training AI</h2>
                <p class="ml-5 text-gray-500">(Hỗ trợ TXT, PDF, XLSX, XLS, CSV. Tối đa 15MB mỗi tệp)</p>
                <button type="button" @click="handleClickUploadDocument"
                    class="ml-auto px-10 flex flex-nowrap items-center font-bold px-1 py-1.5 justify-center gap-2 bg-transparent text-[#207A91] rounded-xl backdrop-blur-sm transition-colors hover:scale-105 border-2 border-[#207A91]">
            ...
            <div class="text-black w-full border rounded-3xl overflow-hidden mb-5 mt-5">
                    ...
                    <tbody class="bg-white">
                        <tr v-for="(item, index) in documents" :key="item.id" :class="item.appointment && 'bg-primary-lurcinn/20'" class="border-b-2 text-xs lg:text-base">
                            ...
                            <td class="text-center">
                                <a class="text-[#033260]" :href="route('chat-training-documents.download-document', {id:item.id})" target="_blank">{{ item.name }}</a>
                            </td>
                            <td>
