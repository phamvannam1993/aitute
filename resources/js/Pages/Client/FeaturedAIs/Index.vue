<template>
    <div class="mt-2 md:mt-0 px-[14px] md:px-0">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-4 px-[20px] md:px-0">
            <div v-for="(ai, index) in displayedFeaturedAIItems" :key="index"
                      class="ai-items bg-white p-[10px] md:p-2 rounded-[10px] md:rounded-2xl relative">
                      <div class="flex items-center justify-between space-x-2 md:mb-2">
                          <div class="w-[60px] h-[60px] md:w-2/8">
                              <img :src="ai?.thumbnail_url || '/assets/svgs/sample-ai-icon-1.svg'" alt="AI Image" class="size-[60px] md:size-[49px] rounded-[13px]" @click="redirectBasedOnType(ai, ai.type)"/>
                          </div>
...
const showMoreFeaturedAIItems = () => {
  ...
  const newItems = aiItems.filter(ai => ai.featured).slice(displayedFeaturedAIItems.value.length, displayedFeaturedAIItems.value.length + 6);
...
const updateFavorites = async (ai) => {
  ...
  response.data.message === 'Added to favorites' ? ai.favorites.push(1) : ai.favorites.pop();
}
