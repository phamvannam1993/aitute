    ...
    <Layout :breadcrumbs="breadcrumbsData">
        ...
        <div class="w-full">
            ...
            <div class="flex justify-between items-center my-5">
                ...
                <a :href="route('text-to-music.index')"
            ...
            <div class="flex-col justify-between ">
                <div class="flex-col justify-between flex">
                    <div
                        class="grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-4 w-full overflow-y-auto my-3 lg:my-0 max-h-[400px] ">
                            ...
                            <div class="w-full relative  pb-2  rounded-xl">
                                <div class="rounded-xl text-black relative group">
                                    <template v-if="item.prompt && item.prompt !== ''">
                                        <p
                                            class="truncate max-w-[250px] overflow-hidden whitespace-nowrap text-ellipsis">
                                            {{ item.prompt }}
                                        </p>
                                        <!-- Tooltip -->
                                        <span
                                ...
                                <div class="bg-gray-400 relative  rounded-md">
                                    <button @click="item.showLyrics = !item.showLyrics" class="p-2">
                                        {{ item.showLyrics ? 'Ẩn Lyrics' : 'Lyrics' }}
                                    </button>
                                    <div
                                        class="absolute group-hover:flex w-fit right-0 bottom-1/2 translate-y-1/2 text-left items-center">
                                        <div class="flex justify-end w-full">
                                            <button @click="prepareDeleteFile(item.id)"
                                                class="p-2 rounded-full cursor-pointer hover:bg-white/30">
                                                <img class="w-5 h-5" src="/assets/svgs/trash-icon-white.svg" />
                                            </button>
                                            <button @click="shareAIGeneratedMedia(item.id)"
                                                class="p-2 rounded-full cursor-pointer hover:bg-white/30">
                                                <img class="w-5 h-5" src="/assets/svgs/share-icon-white.svg" />
                                            </button>
                                            <button @click="downloadMusic(item.audioUrl)"
                                                class="p-2 rounded-full cursor-pointer hover:bg-white/30">
...
const breadcrumbsData = [
    { text: "Lịch sử tạo nhạc", link: "text-to-music.history" },
];
const goBack = () => {
