<template>
    <div :style="{ backgroundColor: data.theme.slideColor }" class="flex justify-start rounded-2xl relative">
        ...
        <div class="">
            <div class="">
                <h1 class="mb-4 mt-4 lg:mb-4 lg:mt-16 mx-auto" :class="isPresent? 'animat-5': '', {paused: props.isPaused}">
                    <editable-content v-bind:isTyping="props.isTyping" v-model="data.title"
                        v-model:customStyle="data.title_style" :is-heading="true" />
                </h1>
            </div>
            <div class="h-[250px] w-[250px] flex flex-col gap-4 ml-24">
                <EditImage :model-value="imgRef[0]" @update:model-value="val => imgRef[0] = val" :title="props.data.title">
                    <img :src="imgRef[0]" alt="img" class="h-[220px] w-[220px] object-fit" :class="isPresent? 'img-1 ': '', {paused: props.isPaused}">
                </EditImage>
                <EditImage :model-value="imgRef[1]" @update:model-value="val => imgRef[1] = val" :title="props.data.title">
                    <img :src="imgRef[1]" alt="img" class="h-[220px] w-[220px] object-fit" :class="isPresent? 'img-2': '', {paused: props.isPaused}">
                </EditImage>
            </div>
        </div>
        <div v-if="props?.virtual?.s3_url" class="w-1/2 absolute bottom-12 right-0 mc-animate">
...
.text {
    ...
    animation-delay: 3s; /* Thời gian trễ 3 giây */
    animation-fill-mode: both; /* Đảm bảo trạng thái ban đầu được giữ */
}

.img-1 {
    animation: slideUp 1.2s ease-out;
    animation-delay: 5s; /* Thời gian trễ 3 giây */
    animation-fill-mode: both; /* Đảm bảo trạng thái ban đầu được giữ */
}
.img-2 {
    animation: slideUp 1.2s ease-out;
    animation-delay: 8s; /* Thời gian trễ 3 giây */
    animation-fill-mode: both; /* Đảm bảo trạng thái ban đầu được giữ */
}
.paused {
