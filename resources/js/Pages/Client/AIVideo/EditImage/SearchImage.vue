    ...
    <form class="max-w-lg mx-auto mb-[20px]" @submit="handleSubmit">
        <div class="flex">
            <button @click="showDropdown = !showDropdown" id="dropdown-button" data-dropdown-toggle="dropdown"
                    class="relative flex-shrink-0 z-10 inline-flex items-center py-1.5 px-3 text-sm font-medium text-black text-center border border-gray-300 rounded-s-lg focus:outline-none"
                    type="button">
                {{ colorName }}
                ...
                <div v-show="showDropdown" id="dropdown" class="z-10 absolute top-[120%] left-0 divide-y divide-gray-500 rounded-lg shadow w-44 bg-white">
                    <ul class="py-1 text-sm text-black" aria-labelledby="dropdown-button">
            ...
            <div class="relative w-full">
                <input v-model="keyword" type="search" id="search-dropdown" class="block p-1.5 pr-3.5 w-full z-20 text-sm text-black rounded-e-lg border border-gray-300 border-s-0 focus:outline-none" placeholder="Nhập từ khoá..." required/>
            </div>
        </div>
    </form>
    <div class="max-h-[314px] overflow-auto p-[12px]">
        <div v-if="isLoading && searchImages.length == 0" class="flex items-center justify-center z-[60]">
            <div class="loader"></div>
        </div>
        <div class="columns-5 gap-4">
            <div v-for="(image, k) in searchImages" :key="k" class="mb-4 break-inside-avoid rounded shadow cursor-pointer" @click="handleImageSelect(image.link)">
                <img :src="image.link" class="w-full h-auto rounded-md object-cover" :alt="image.title"/>
            </div>
        </div>

        <div class="text-center mt-[24px]">
            <button v-if="hasMore" type="button" class="py-2.5 px-5 me-2 text-sm font-medium text-black bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:outline-none focus:ring-blue-700 focus:text-blue-700  inline-flex items-center" @click="handleNextPage">
                ...
                <span class="text-black">{{ isLoading ? "Loading..." : "Xem thêm" }}</span>
            </button>
        </div>
    </div>
</template>
...
const callSearchImage = async () => {
    try {
        ...
        const response = await axios.get(route("image.search"), {
            params: {
