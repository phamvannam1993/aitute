    ...
    >
        ...
        <div class="absolute inset-0 opacity-0 group-hover:opacity-[1] duration-300 delay-[150] transition-all">
            <div class="flex gap-[8px] items-center justify-center h-full">
                <div class="w-[36px] h-[36px] p-[8px] rounded-[50%] border border-white text-white cursor-pointer hover:bg-white hover:text-black duration-300 transition-all" 
    ...
    <div v-show="showEditPopup" 
        ...
        <div class="text-sm font-medium text-center text-gray-500 border-b border-gray-200">
            <ul class="flex flex-wrap -mb-px">
                <li class="me-2" @click="activeTab = 'search_tab'">
                    <span class="cursor-pointer inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300"
                          :class="{ '!border-blue-600 text-blue-600': activeTab === 'search_tab' }">
                        Tìm kiếm
                    </span>
                </li>
        ...
        <div class="h-[400px] w-[350px] sm:w-[500px] md:w-[600px] p-[20px] overflow-y-auto">
            ...
            <div v-show="activeTab === 'local_upload_tab'">
                <UploadImage v-model="imageLink" @imageUploaded="handleClosePopup" />
            </div>
        </div>
    </div>
</template>
...
const DEFAULT_IMAGE = 'https://c.pxhere.com/photos/ef/9f/flying_bird_bird_flight_wing_sky-30166.jpg!s2';

const handleImageSelected = (link) => {
    imageLink.value = link;
    emit('urlSubmitted', link);
    handleClosePopup();
};
...
const resetToDefault = () => {
    imageLink.value = DEFAULT_IMAGE;
    emit('urlSubmitted', DEFAULT_IMAGE);
};
