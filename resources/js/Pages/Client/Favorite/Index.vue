<template>
  <Head title="Yêu thích - AI 3 GỐC - Cộng đồng AI tử tế" />
      ...
      <div class="mt-[48px] md:p-[34px] md:mt-2" >
        <header class="flex items-center justify-between px-2">
          <div class="flex items-center">
            ...
            <span class="text-[14px] text-[#2C75E3] font-medium ml-2"> / Yêu thích</span>
          </div>
         ...
         <div class="flex flex-1 flex-col mt-[12px] md:mt-[8px]">
           <div class="flex flex-col xl:flex-row my-2 xl:space-x-6 text-black">
               <div class="px-[14px] md:px-0 xl:w-3/4 mb-4">
                    <h1 class="font-bold text-[25px] text-black leading-[52px]">Yêu thích</h1>
...
const displayedAssistants = ref(props.favoriteAssistants);
console.log(props.favoriteAssistants);
...
const searchAI = async () => {
  try {
    ...
    const response = await axios.post(route('favorites.search'), { search: searchQuery.value });
...
const showMoreItems = async () => {
  try {
    ...
    const response = await axios.post(route('favorites.load-more'), { search: searchQuery.value, offset: offset.value });
