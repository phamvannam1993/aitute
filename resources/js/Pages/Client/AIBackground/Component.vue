            ...
            <section class="w-full flex flex-col lg:w-5/12 bg-white p-3 lg:p-4 2xl:p-6 rounded-3xl text-black drop-shadow-xl">
                ...
                <div class="flex flex-col gap-2 items-center my-2 mb-4">
                    <div class="flex gap-2 items-center justify-between w-full">
                        <label for="image-description" class="text-xs lg:text-sm flex gap-1 items-center mb-1">
                            <div class="flex items-center py-1 px-2 rounded-2xl text-white bg-orion-sec">
                        ...
                        <button type="button" @click="handleUploadClick"
                            class="w-[28%] xs:w-1/4 xl:w-1/3 justify-self-end flex flex-nowrap items-center font-bold px-1 py-1.5 justify-center gap-2 bg-transparent text-oriobg-orion-primary rounded-xl backdrop-blur-sm transition-colors hover:scale-105 border-2 border-oriobg-orion-primary">
                            <div class=" p-1 rounded-full bg-orion-sec">
                ...
                <label for="image-description" class="text-xs lg:text-sm flex gap-1 items-center mb-1 font-semibold mt-2">
                    <div class="flex items-center py-1 px-2 rounded-2xl text-white bg-orion-sec">
                        <img src="/assets/img/orion/icon/label-arrow.svg" alt="step" class="w-5 h-auto" />
                        <p>Bước 2</p>
                    </div>
                    <p>Chọn mẫu bối cảnh phù hợp</p>
                </label>
                <label for="typeGernerate-image" class="flex itmems-center gap-2 my-4">
                    <input id="typeGernerate-image" type="checkbox" @input="handleChangeTypeGernerate" :checked="typeGernerate === 'image'" value="image"
                        class="rounded-full w-[18px] h-[18px] flex-shrink-0 checked:bg-orion-primary text-oriobg-orion-primary">
                ...
                <div class="w-fit max-h-[600px] md:max-h-[810px] overflow-y-auto rounded-lg relative text-center mt-2" @mousedown.prevent>
                    <div class="grid grid-cols-2 gap-1 w-full">
                        <label for="image-file" v-if="!backgroundTemp"
                            class="h-[200px] md:h-[270px] flex items-center justify-center relative cursor-pointer w-full rounded-xl bg-[#E6E6E6] text-orion-primary"
                            :class="backgroundImageUrl == null ? 'border-4' : ''">
                            <div class="flex flex-col gap-2 inset-0 m-auto items-center">
                                <img src="/assets/img/upload.png" class="size-6 md:size-8 xl:size-12" />
                                <span class="text-xs md:text-[20px]">Tải ảnh lên</span>
                            </div>
                            <input type="file" id="image-file" @change="handleBackgroundImageFileChange" accept="image/*" class="hidden" />
                        </label>
                        <div v-else class="h-[200px] md:h-[270px] relative w-full rounded-lg border-4 border-oriobg-orion-primary">
                        ...
                        <div @click="handleChangeTemplate(template)" v-for="template in themes" :key="template.id" class="h-[200px] md:h-[270px] relative rounded-xl cursor-pointer transition-all "
                            :class="backgroundImageUrl == template.thumbnail ? 'border-4 border-orion-orange bg-orion-primary' : 'border-2 border-white'">
                            <img
                ...
                <div class="relative ">
                    ...
                    <textarea id="image-description" v-model="imageDescription" type="text" placeholder="Nhập nội dung cần tạo ảnh dưới đây..."
                        ...
                        class="w-full p-3 h-[200px] border text-black border-oriobg-orion-primary rounded-[10px] shadow-sm focus:outline-none focus:ring-2 focus:ring-oriobg-orion-primary"></textarea>
                </div>
                <div class="grid grid-cols-2 mt-4">
                    ...
                    <button type="button" :disabled="isLoading" @click="generateAiBackground"
                        class="font-bold flex items-center w-full justify-center gap-2 px-3 py-2 bg-orion-primary text-white rounded-xl backdrop-blur-sm transition-colors hover:scale-105 border-2 border-oriobg-orion-primary">
                        <span class="text-[12px] xl:text-[14px]">Tạo bối cảnh</span>
                    </button>
                </div>
            </section>

            <section ref="resultBox" class="w-full lg:w-7/12 bg-white rounded-3xl text-black drop-shadow-xl p-3 lg:p-4 2xl:p-6">
                <p class="text-oriobg-orion-primary italic font-thin">Hiển thị kết quả</p>
                <div id="result" class=" flex flex-col gap-3">
                    <div class="h-full">
                ...
                <div class="p-2 flex flex-col gap-2">
                    ...
                    <div ref="scrollContainer" class="flex gap-4 overflow-auto" style="scroll-behavior: smooth">
                        ...
                        <div v-for="(item, index) in listHistory.data" :key="index" class="h-32 w-32 bg-gray-300 rounded-md flex-shrink-0 cursor-pointer">
...
const breadcrumbsData = [
    { text: "Ảnh", link: "ai-background.history" },
...
const fetchThemes = async () => {
    try {
        const response = await axios.get(route('background-images.component'));
        console.log(response.data);
        themes.value = response.data.backgroundImages.map(item => ({
            label: item.category,
            thumbnail: item.sample_url,
            originalLabel: item.category
        }));
        listHistory.value = response.data.history;
    } catch (error) {
...
const generateAiBackground = async () => {
    try {
        ...
        if (res.status === 200) {
            ...
            listHistory.value.data.unshift(res.data);
            credits.value = res.data.credits;
            emit('saveGenerationResult', res)
        }
    } catch (error) {
        console.error(error);
