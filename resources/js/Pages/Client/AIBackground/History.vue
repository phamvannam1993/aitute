<template>
    <Head title="Lịch sử tạo hình bối cảnh - AI 3 GỐC - Cộng đồng AI tử tế" />
                    ...
                    <Layout :breadcrumbs="breadcrumbsData" :credits="credits">
                        ...
                        >
                            <h2 class="text-orion-primary font-bold text-2xl mb-4">
                                Lịch sử tạo hình bối cảnh
                            </h2>
                            <a
                                :href="route('ai-background.indexV2')"
...
const fetchData = async (page) => {
  try {
    ...
    const response = await axios.get(route('ai-background.history', { page }));
...
const confirmDelete = async () => {
    ...
    try {
        const response = await axios.post(
            route("ai-background.delete", { id: imageDeleteId.value })
        );
...
const shareAIGeneratedMedia = async (item) => {
    try {
        const response = await axios.post(route("share-link.store"), {
            ...
            share_linkable_type: 'PebblelyVideo',
        });
