        ...
        <div class="w-full">
            ...
            <!-- <div class="flex justify-center gap-2 md:gap-4 mb-4 h-[40px]">
                 <button @click="autoCreateImage"
                    ...
                    class="w-[35%] sm:w-1/3 bg-[#0099FF] text-white py-2.5 text-xs font-bold lg:text-sm rounded-2xl lg:rounded-2xl text-center hover:bg-cyan-600 flex items-center gap-2 justify-center">
                    Tự động tạo ảnh {{  showImageGenerate == true ? '...' : '' }}
                </button>
                <label id="image_0" class="w-[35%] sm:w-1/3 bg-[#0099FF] text-white py-2.5 text-xs font-bold lg:text-sm rounded-2xl lg:rounded-2xl text-center hover:bg-cyan-600 flex items-center gap-2 justify-center">
                    ...
                    <label id="image_upload" class="cursor-pointer bg-primary-luccin text-white py-2 text-xs font-bold lg:text-sm rounded-2xl lg:rounded-2xl text-center flex items-center gap-2 justify-center">
                        <Upload size="16" color="#ffffff" class="hidden md:show" />
                        <img src="/assets/svgs/aicon/uploadMedia.svg" alt="" />
                        Tải ảnh
                ...
                <div class="relative w-[30%] sm:w-1/3 text-center">
                    <button @click="isGenerateImage = !isGenerateImage"  class="w-full h-full bg-[#0099FF] text-white font-bold py-2.5 text-xs lg:text-sm rounded-2xl lg:rounded-2xl text-center hover:bg-cyan-600 flex items-center gap-2 justify-center">
            ...
            <div class="grid grid-cols-2 items-center justify-center w-full gap-2">
                <div class="flex flex-col justify-center items-center h-full w-full rounded-xl overflow-hidden min-h-[400px]" v-for="(image, index_image) in images" :key="index_image">
                    <div class="relative m-auto" v-if="image.imageRef.s3_url && !image.isLoading">
                        <input v-if="image.imageRef.s3_url" type="checkbox" :checked="checkImage(index_image)" @click="selectImage(index_image)" class="ml-0 top-2 left-2 absolute rounded-full p-2 checked:bg-[#FFA411] checked:ring-1 checked:ring-[#FFA411] ring-offset-2 ring-offset-white checked:hover:bg-[#FFA411]" />
        ...
        <div class="bg-white w-[80vw] md:w-[50vw] p-4 rounded-xl">
            <div class="flex items-center justify-between">
                <div class="flex justify-start items-center gap-4">
                    <img src="/assets/img/aiwow-1.png" alt="step" class="w-[24px] h-auto" />
                    <span class="text-[15px] lg:text-[18px] leading-none font-bold text-black">Tìm kiếm hình ảnh</span>
                </div>
                <button type="button" @click="showSearchModal = false" class="p-1 rounded-full">
                    <img src="/assets/svgs/aiwow/remove_icon.svg"/>
                </button>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4 max-h-[60vh] overflow-y-auto">
                ...
                <div class="flex justify-center items-center">
                    <button type="button" @click="loadMoreImages" :disabled="isLoadingMore" :class="isLoadingMore ? 'bg-opacity-60' : 'bg-opacity-100'" class="min-w-[100px] md:min-w-[150px] h-fit px-2 py-2 lg rounded-lg lg:rounded-2xl border border-[#C5C5C5] text-white font-bold place-items-center gap-2 text-sm bg-primary-color">{{ isLoadingMore ? 'Đang tải thêm ảnh...' : 'Xem thêm...' }} </button>
                </div>
            </div>
            <div class="flex justify-center items-center my-4">
                <button @click="confirmSelectedImage" class="min-w-[100px] md:min-w-[150px] px-2 py-2 lg bg-aiwow-primary rounded-lg lg:rounded-2xl border border-[#C5C5C5] text-white font-bold place-items-center gap-2 text-sm">Xác nhận</button>
            </div>
        </div>
    </div>
</template>
...
<script setup>
    ...
    const updateDataDetail = (data) => {
        ...
        images.value = data.results.images
    }
    ...
    const autoCreateImage = async(isAuto = false) => {
        var modelProject = selectedProject.value?.image_model
        ...
        for(var idx = 0; idx < images.value.length; idx++) {
            ...
            if(idx == index) {
                ...
                if(images.value[i].type_image  == 1) {
                    ...
                    s3_url = await createAIImage(index)
    ...
    const createAIImage = async (index) => {
        let s3_url = '';
        try {
            const content = results.value.content
            const response = await axios.post(route("ai-image.generate"), {
    ...
    const createImageV2 = async (imageProduct, modelProduct) => {
        let formData = new FormData();
        formData.append("image_model", modelProduct);
    ...
    defineExpose({ saveImage, saveGenerationResult, updateDataDetail, updateContent, autoCreateImage });
</script>
