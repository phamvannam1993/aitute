        ...
        <div>
            ...
            <div  class="my-8 flex justify-center items-center gap-6">
                <button type="button" @click="handlePostFacebook(1)"
                    class="min-w-[70px] text-center inline-flex gap-2 items-center justify-center w-44 px-2 py-2 lg:py-3 btn-gray rounded-lg lg:rounded-2xl border border-[#C5C5C5] text-white font-bold">
                    Lưu nháp
                </button>
                <button type="button" @click="handlePostFacebook(2)"
                    class="min-w-[90px] text-center inline-flex gap-2 items-center justify-center w-44 px-2 py-2 lg:py-3 btn-red rounded-lg lg:rounded-2xl border border-[#C5C5C5] text-white font-bold">
                    Không duyệt
                </button>
                <button type="button" @click="handlePostFacebook(3)"
                    class="min-w-[90px] flex items-center justify-center gap-2 px-2 md:px-4 py-2 lg:py-3 btn-green text-white rounded-lg lg:rounded-2xl font-bold text-sm">
                    Duyệt bài
                </button>
            </div>
        </div>
...
const handlePostFacebook = (status) => {
    if (status === 3 && fanpages.value.length < 1) {
        toast.error(
...
const updateData = (status = "") => {
    ...
    emit('updateFacebookContent', formData, status, published.value, facebookContentDetail.value.id);
}
</script>
<style>
    .btn-red {
