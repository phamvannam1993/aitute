<template>
      <div class="flex-col lg:rounded-[35px] bg-white shadow-xl md:px-[80px] md:py-[20px] p-3 flex items-center justify-between mt-4">
        ...
        <div class="w-full mt-10 md:mt-5 text-black text-xs lg:text-[15px] relative" :class="open ? '' : 'hidden'">
            ...
            <ImageSection ref="refImage" :results="results" :selectedProject="selectedProject" @updateResult="handleUpdateResult" @updateProject="updateProjectExpert"/>
            <VideoSection @updateResult="handleUpdateResult" ref="childRef" @showFormVideo="handleShowFormVideo" :results="results" @updateProject="updateProjectExpert"/>
        </div>
        <div class="w-full mt-10 md:mt-5 text-black text-xs lg:text-[15px] relative" v-if="isShowVideo && open" >
        ...
        <div class="flex justify-center gap-4 mt-4 mb-4" v-if="open">
            <button type="submit" class="h-[40px] md:h-[50px] min-w-[80%] lg:min-w-[360px] text-sm lg:text-base px-4 bg-aiwow-sec text-white rounded-lg lg:rounded-2xl font-bold" :disabled="loading"  @click="showPostForm">
...
<script setup>
    ...
    import ContentSection from '@/Pages/Client/MyAssistants/ProjectAdvance/TypeCampaign/ChildResults/ContentSection.vue';
    import ImageSection from '@/Pages/Client/MyAssistants/ProjectAdvance/TypeCampaign/ChildResults/ImageSection.vue';
    import VideoSection from '@/Pages/Client/MyAssistants/ProjectAdvance/TypeCampaign/ChildResults/VideoSection.vue';
    ...
    const updateDataDetail = (data) => {
        ...
        if(refImage.value) {
            refImage.value.updateDataDetail(data)
        }
        if(contentSectionRef.value) {
            contentSectionRef.value.updateContent(data.results.content)
        }
        if(childRef.value) {
            childRef.value.updateDataDetail(data)
        }
    }

    const updateContent = (content, isUpdate = false) => {
        if(refImage.value) {
            refImage.value.updateContent(content)
            if(isUpdate) {
                refImage.value.autoCreateImage(true)
            }
        }
        if(childRef.value) {
            childRef.value.updateContent(content)
        }
        if(contentSectionRef.value) {
    ...
    defineExpose({  updateDataDetail, updateContent });
</script>
