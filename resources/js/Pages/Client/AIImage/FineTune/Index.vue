<template>
    <Head title="Đào tạo mẫu ảnh của tôi - AI 3 GỐC - Cộng đồng AI tử tế" />
        <Layout :breadcrumbs="breadcrumbsData" :credits="credits" isBig>
            <div class="flex justify-center gap-4 w-full flex-wrap md:flex-nowrap border-[3px] border-[#d4d4d4] rounded-[25px] bg-white">
                <div ref="resultBox" class="bg-white w-full rounded-[10px] md:rounded-[25px] lg:w-4/5 p-[12px] md:p-[25px] h-fit">
                    ...
                    <form @submit.prevent="handleSubmit" class="">
                                        ...
                                        <div class="w-full">
                                            <div class="flex flex-col lg:flex-row items-center justify-start lg:justify-between gap-1 w-full">
                                                <div class="flex items-center">
                                                    <Step :step="1" stepName="Tải hình ảnh của bạn (theo yêu cầu)" forName="image-description" />
                                            ...
                                            <span class="italic text-[12px]  text-ai3goc-primary whitespace-pre-line">Lưu ý:
                                            <br>- Hình ảnh để đào tạo AI phải là ảnh gốc không qua chỉnh sửa. 
                                            <br>- Nếu chỉnh sửa thì chỉ xóa những chi tiết quá xấu trên khuôn mặt như mụn trứng cá, không được xóa những đặc trưng khuôn mặt như nốt ruồi... 
                                            <br>- Phải chọn đủ 10 ảnh đẹp nhất đủ góc cạnh gương mặt, tư thế, biểu cảm, trang phục để đạt được kết quả ảnh đẹp nhất.</span>

                                        </div>
                                        
                                        <!-- <label for="select-avatar" class="">
                                            <div class="relative">
                                                <input type="file" id="avatar" @change="handleAvatarChange" accept=".jpg,.png,.jpeg" class="hidden" ref="avatarInput" multiple />
                                                <button type="button" @click="$refs.avatarInput.click()" class="flex items-center min-w-48 w-full md:w-fit border-[2px] border-primary-lurcinn justify-center gap-2 py-1 px-8 bg-white hover:bg-black/10 text-primary-lurcinn rounded-lg backdrop-blur-sm transition-colors">
                                                    <img src="/assets/img/aicon/icon/features/upload.png" class="size-6 md:size-5 xl:size-6" />
                                                    <span class="text-[12px] xl:text-[16px]">Tải ảnh lên</span>
                                                </button>
                                            </div>
                                        </label> -->
                                    </div>
                                ...
                                <!-- <div class="mt-4">
                                    <div class="flex items-center gap-2">
                                        <img src="/assets/img/aicon/logo/step.png" alt="step" class="w-5 h-auto" />
                                        <p class="text-black font-semibold text-sm">Danh sách ảnh đã tải lên (<span class="text-primary-lurcinn">{{ uploadedImages.length }}</span>/10):</p>
                                    </div>
                                    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 mt-2">
                                        <div v-for="(image, index) in uploadedImages" :key="index" class="relative flex items-center justify-center rounded-[12px] overflow-hidden shadow-lg">
                                            <img :src="image.preview" alt="Uploaded Image" class="aspect-auto max-h-[180px] object-cover rounded-lg" />
                                            <button type="button" @click="removeImage(index)" class="absolute top-1 right-1 bg-white text-white rounded-full w-[24px] h-[24px] flex justify-center items-center hover:bg-red-700">
                                                <img src="/assets/img/aicon/icon/close.png" alt="Close" class="w-6 h-auto" />
                                            </button>
                                        </div>

                                        <div v-for="index in 10 - uploadedImages.length" :key="'placeholder-' + index" class="flex items-center justify-center">
                                            <div class="bg-[#E6E6E6] flex items-center justify-center aspect-square h-[140px] md:h-[180px] rounded-lg shadow-lg">
                                                <img src="/assets/img/aicon/icon/placeholder-face.png" alt="placeholder" class="mx-auto w-24" />
                                            </div>
                                        </div>
                                    </div>
                                </div> -->
                                <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 md:gap-4 gap-2 mt-2">
                                    <div
                                        v-for="(slot, index) in slots"
                                        :key="index"
                                        class="relative flex items-center justify-center rounded-[12px] overflow-hidden"
                                    >
                                        <div v-if="uploadedImages[index]" class="relative">
                                            <img
                                                :src="uploadedImages[index].preview"
                                                alt="Uploaded Image"
                                                class="aspect-square w-[150px] md:w-[180px] h-[150px] md:h-[180px] object-cover rounded-lg"
                                            />
                                            <button
                                                type="button"
                                                @click="removeImage(index)"
                                                class="absolute top-1 right-1 bg-white text-white rounded-full w-[24px] h-[24px] flex justify-center items-center hover:bg-red-700"
                                            >
                                                <img src="/assets/img/orion/icon/close.svg" alt="Close" class="w-6 h-auto" />
                                            </button>
                                        </div>

                                        <div v-else class="bg-[#E6E6E6] flex flex-col items-center justify-center aspect-square w-[150px] md:w-[180px] h-[150px] md:h-[180px] rounded-lg shadow-lg">
                                            <button type="button"
                                                @click="triggerInput(index)"
                                                class="text-ai3goc-primary text-sm font-bold mt-3"
                                            >
                                                <img src="/assets/img/ai3goc/icon/upload.svg" class="max-sm:w-10 w-14" alt="">
                                            </button>
                                            <div class="mt-auto mb-2 bg-[#F0F0F0] mx-2 rounded-lg w-11/12 min-h-[80px] text-center">
                                                <p class="text-[10px] md:text-xs lg:text-[14px] p-1 text-black">{{ slot.label }}</p>
                                ...
                                <div class="my-4 flex flex-col md:flex-row md:items-center gap-2 justify-between">
                                    ...
                                    <input id="modelName" v-model="modelName" type="text" minlength="3" maxlength="50" class="block mt-2 text-[14px] appearance-none md:w-5/12 text-black py-2 px-4 pr-8 rounded-[10px] leading-tight focus:outline-none bg-white border-ai3goc-primary" placeholder="Đặt tên cho mô hình AI của bạn"/>
                                </div>
                                <div class="flex flex-col md:flex-row text-black md:items-center justify-between gap-2 mt-4 w-full">
    ...
    <PopupN 
        v-if="isShowTrainingModel"
        title="Xin chào!"
        :message="'<p>Mẫu ảnh A.I của bạn đang được đào tạo, quá trình đào tạo sẽ diễn ra trong khoảng 30-45 phút.</p><p>Xin vui lòng chờ!</p>'"
        cancelButtonText="Bỏ qua"
        @cancel="isShowTrainingModel = false"
    />
...
const images = ref([
    { url: "/assets/img/orion/training_image/1.jpg" },
    { url: "/assets/img/orion/training_image/5.jpg" },
    { url: "/assets/img/orion/training_image/2.jpg" },
    { url: "/assets/img/orion/training_image/6.jpg" },
    { url: "/assets/img/orion/training_image/4.jpg" },
    { url: "/assets/img/orion/training_image/8.jpg" },
    { url: "/assets/img/orion/training_image/10.jpg" },
    { url: "/assets/img/orion/training_image/7.jpg" },
    { url: "/assets/img/orion/training_image/9.jpg" },
    { url: "/assets/img/orion/training_image/3.jpg" },
]);
...
const slots = ref([
    { label: "Ảnh 1: Ảnh chụp cận mặt, góc chính diện, hình ảnh rõ nét các bộ phận trên gương mặt", condition: (file) => true },
    { label: "Ảnh 2: Ảnh chụp cận mặt, góc chính diện (nên dùng ảnh có kiểu tóc khác)", condition: (file) => true },
    { label: "Ảnh 3: Ảnh chụp cận mặt, góc 1/2, hình ảnh rõ nét các bộ phận trên gương mặt", condition: (file) => true },
    { label: "Ảnh 4: Ảnh chụp cận mặt, góc 3/4, hình ảnh rõ nét các bộ phận trên gương mặt", condition: (file) => true },
    { label: "Ảnh 5: Ảnh chụp nửa thân trên, góc chính diện, hình ảnh rõ nét", condition: (file) => true },
    { label: "Ảnh 6: Ảnh chụp nửa thân trên, góc chính diện (nên dùng ảnh có kiểu tóc và trang phục khác)", condition: (file) => true },
    { label: "Ảnh 7: Ảnh chụp nửa thân trên, góc 3/4, hình ảnh rõ nét", condition: (file) => true },
    { label: "Ảnh 8: Ảnh chụp nửa thân trên, 3/4 (nên dùng ảnh có kiểu tóc và trang phục khác)", condition: (file) => true },
    { label: "Ảnh 9: Ảnh chụp toàn thân, góc chính diện", condition: (file) => true },
    { label: "Ảnh 10: Ảnh chụp toàn thân, góc nghiêng 3/4", condition: (file) => true },
]);
...
const handleSubmit = async () => {
    ...
    processedImages.flat().forEach(({ file, index, subIndex }) => {
        zip.file(
            `processed_image_${index + 1}_${subIndex + 1}.${file.name.split('.').pop()}`,
            file
        );
    });
            ...
            const createFlippedImage = () => {
                ...
                canvas.toBlob((blob) => {
                    processedImages.push(
                        new File(
                            [blob],
                            `${image.file.name.split('.')[0]}_flipped.${image.file.name.split('.').pop()}`
                        )
                    );
                });
            };
