<template>
    ...
    <div :class="!props.isActive ? 'pointer-events-none opacity-70' : ''" class="flex gap-1 justify-around w-full flex-wrap mt-2">
        <!-- <button type="button" @click="props.handleSharpenClick && props.handleSharpenClick()"
            ...
            <img src="/assets/img/aiwow/studio.png" alt="AI Video" class="max-lg:w-4 hidden md:block"
                class="min-w-[20px] w-[32px] h-[32px] lg:w-[28px] lg:h-[28px]" />
            <span
                class="font-medium hover:scale-105 text-white px-1 lg:px-1 xl:px-2 py-1.5 text-sm text-nowrap lg:text-sm">Làm
                nét</span>
        </button> -->
        <a v-if="props?.features?.includes('video')"
            ...
            <img src="/assets/svgs/aiwow/taskbar/video.svg" alt="AI Video" class="max-lg:w-4 hidden md:block" />
            <span
                class="font-medium hover:scale-105 text-white px-1 lg:px-1 xl:px-2 py-1.5 text-sm text-nowrap lg:text-sm">Video</span>
        </a>
        <a v-if="props?.features?.includes('background')"
            :href="route('ai-background.indexV2', { imageUrl: props.selectedImage?.s3_url })"
            class="relative group md:min-w-[120px] justify-center rounded-full bg-[#149CBE] flex gap-1 px-2 items-center">
            <img src="/assets/svgs/aiwow/taskbar/background.svg" alt="AI Video" class="max-lg:w-4 hidden md:block"/>
            <span
                class="font-medium hover:scale-105 text-white px-1 lg:px-1 xl:px-2 py-1.5 text-sm text-nowrap lg:text-sm">Phối cảnh</span>
        </a>
        <a v-if="props?.features?.includes('lipsync')"
            :href="route('lipsync.lipsync', { videoUrl: props.selectedImage?.s3_url })"
            class="relative group md:min-w-[120px] justify-center rounded-full bg-[#149CBE] flex gap-1 px-2 items-center">
            <img src="/assets/svgs/aiwow/taskbar/lipsync.svg" alt="AI Video" class="max-lg:w-4 hidden md:block" />
            <span
                class="font-medium hover:scale-105 text-white px-1 lg:px-1 xl:px-2 py-1.5 text-sm text-nowrap lg:text-sm">Lồng
                tiếng</span>
        </a>
        <a v-if="props?.features?.includes('swap_face')"
            :href="route('ai-image.generate-swap-face', { imageUrl: props.selectedImage?.s3_url })"
            class="relative group md:min-w-[120px] justify-center rounded-full bg-[#149CBE] flex gap-1 px-2 items-center">
            <img src="/assets/svgs/aiwow/taskbar/faceswap.svg" alt="AI Video" class="max-lg:w-4 hidden md:block" />
            <span
                class="font-medium hover:scale-105 text-white px-1 lg:px-1 xl:px-2 py-1.5 text-sm text-nowrap lg:text-sm">Đổi
                mặt</span>
        </a>
        <a v-if="props?.features?.includes('background_audio')" @click="$refs.fileInputAudio.click()"
            ...
            <img v-if="!isCallUploadAudio" src="/assets/svgs/aiwow/taskbar/music.svg" alt="AI Video" class="max-lg:w-4 hidden md:block"/>
            <span
                class="font-medium hover:scale-105 text-white px-1 lg:px-1 xl:px-2 py-1.5 text-sm text-nowrap lg:text-sm">Nhạc
                nền</span>
        ...
        <button type="button" v-if="props?.features?.includes('background_music')"
            class="relative group md:min-w-[120px] justify-center rounded-full bg-[#149CBE] flex gap-1 px-2 items-center">
            <img src="/assets/svgs/aiwow/taskbar/music.svg" alt="AI Video" class="max-lg:w-4 hidden md:block" />
            <span
                class="font-medium hover:scale-105 text-white px-1 lg:px-1 xl:px-2 py-1.5 text-sm text-nowrap lg:text-sm">Nhạc
                nền</span>
        </button>
        <div class="relative text-nowrap lg:min-w-[120px]">
            <button type="button" @click="toggleShowMore"
                ...
                <img src="/assets/svgs/aiwow/taskbar/more.svg" alt="AI Video" class="w-4 h-4 hidden md:block" />
                <span class="font-medium max-lg:text-xs px-1 text-nowrap lg:text-sm hover:scale-105 rounded-md text-white">Khác</span>
            </button>
            <div v-if="isShowMore"
                class="bg-white text-black menu-container absolute top-[120%] w-[10rem] right-0 z-[100] border-2 border-[#E6E6E6] gap-4 flex flex-col p-3 rounded-[20px]">
