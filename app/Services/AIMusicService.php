...
{
    private $aIMusicRepository;
    public function __construct(AIMusicRepository $aIMusicRepository)
    {
        $this->aIMusicRepository = $aIMusicRepository;
    }
    ...
    {
        try {
            $message = $this->aIMusicRepository->create($data);
            if (!$message) {
    ...
    public function getListMedia($type = 'image')
    {
        try {
            $userId = auth('web')->id();
            $per_page = 2;
    ...
    public function saveSwapFace(string $s3Path, string $sourceImg, string $targetImg): AIGeneratedMedia
{
