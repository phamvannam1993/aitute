...
class DifyService
{
    ...
    {
        $this->appKey = config('services.self_host_dify.api_key');
        $this->appKeyV2 = config('services.self_host_dify.api_key_v2');
        $this->baseUrl = config('services.self_host_dify.base_url');

        $this->selfHostAppKey = config('services.dify.api_key');
        $this->selfHostAppKeyV2 = config('services.dify.api_key_v2');
        $this->selfHostBaseUrl = config('services.dify.base_url');
    }
    ...
    public function sendChat($options)
    {
        try {
            ...
            $data['response'] = $client->post($chatUrl, $options);
