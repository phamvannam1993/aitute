...
class Client
{
    ...
    public function sendRequest(Request $request)
    {
        ...
        //Start Override
        //vendor\joelbutcher\facebook-graph-sdk\src\Client.php
        $requestFactory = Psr17FactoryDiscovery::findRequestFactory();
        $guzzleHttprequest = $requestFactory->createRequest($method, $url);
        foreach ($headers as $header => $value) {
            $guzzleHttprequest = $guzzleHttprequest->withHeader($header, $value);
        }
        if ($body) {
            $streamFactory = Psr17FactoryDiscovery::findStreamFactory();
            $bodyStream = $streamFactory->createStream($body);
            $guzzleHttprequest = $guzzleHttprequest->withBody($bodyStream);
        }
        $psr7Response = $this->httpClient->sendRequest($guzzleHttprequest);
        //End
